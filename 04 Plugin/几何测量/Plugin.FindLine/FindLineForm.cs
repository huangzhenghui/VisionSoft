...
{
    public partial class FindLineForm : FormBase
    {
        #region 字段、属性
        ...
        public FindLineForm(FindLineObj Obj) : base(Obj)
        {
            InitializeComponent();
            mObj = Obj;
            ...
            chxShowLine.ForeColor = Color.FromArgb(100, 100, 100);
        ...
        public void ShowHObj()
        {
            ...
            {
                mWindowH.WindowH.DispROI(mObj.ModInfo.Name + ROIType.FindLine, mRoiList[mObj.ModInfo.Name + ROIType.FindLine]);
            }
            else
            {
                if (FindLineObj.fittedPointCopy != null && FindLineObj.fittedPointCopy.IsInitialized())
                {
                    HObject hObj = FindLineObj.fittedPointCopy.Clone();
                ...
                if (FindLineObj.fittedPointCopy != null && FindLineObj.removedPointCopy.IsInitialized())
                {
                    HObject hObj = FindLineObj.removedPointCopy.Clone();
                    HOperatorSet.SetLineWidth(mWindowH.HWindowHalconID, 1.45);
                ...
                if (FindLineObj.fittedPointCopy != null && FindLineObj.lineCopy.IsInitialized())
                {
                    HObject hObj = FindLineObj.lineCopy.Clone();
                    HOperatorSet.SetLineWidth(mWindowH.HWindowHalconID, 1.45);
        ...
        public void InitVar()
        {
            ...
            mObj.strRowBegin = (height / 2).ToString();
            mObj.strColBegin = (width / 3).ToString();
            mObj.strRowEnd = (height / 2).ToString();
            mObj.strColEnd = (width * 2 / 3).ToString();
        ...
        public void GenROI()
        {
            ShowImage();
            if (mObj.mRImage == null || !mObj.mRImage.IsInitialized()) return;
            mWindowH.WindowH.GenFindLine(mObj.ModInfo.Name + ROIType.FindLine, mObj.rowBegin, mObj.colBegin, mObj.rowEnd, mObj.colEnd, mObj.measureNum, mObj.length1, mObj.length2, ref mRoiList, mObj.projectColor, mObj.meaRectColor);
        }
        ...
        public override void FormToObj()
        {
            ...
            mObj.strRowBegin = ldBeginY.TextInfo.Trim();
            mObj.strColBegin = ldBeginX.TextInfo.Trim();
            mObj.strRowEnd = ldEndY.TextInfo.Trim();
            mObj.strColEnd = ldEndX.TextInfo.Trim();
            ...
            mObj.isShowReg = chxShowLine.Checked;
            ...
            mObj.resultColor = ColorTranslator.ToHtml(cpLine.Value);
        ...
        public override void ObjToForm()
        {
            ...
            ldBeginX.TextInfo = mObj.strColBegin;
            ldBeginY.TextInfo = mObj.strRowBegin;
            ldEndX.TextInfo = mObj.strColEnd;
            ldEndY.TextInfo = mObj.strRowEnd;
        ...
        private void Hwindow_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            ldBeginX.TextInfo = ((ROIFindLine)roi).colBegin.ToString();
            ldBeginY.TextInfo = ((ROIFindLine)roi).rowBegin.ToString();
            ldEndX.TextInfo = ((ROIFindLine)roi).colEnd.ToString();
            ldEndY.TextInfo = ((ROIFindLine)roi).rowEnd.ToString();
            tbxLength1.TextInfo = ((ROIFindLine)roi).length1.ToString();
            tbxLength2.TextInfo = ((ROIFindLine)roi).length2.ToString();
        }

...
{
    public partial class HObjectSelForm : FormBase
    {
        #region 字段、属性
        ...
        public HObjectSelForm(HObjectSelObj Obj) : base(Obj)
        {
            InitializeComponent();
            tbxIdx.RectColor = Color.AliceBlue;
            tbxIdx.FillColor = Color.AliceBlue;
            tbxIdx.ForeColor = Color.FromArgb(100, 100, 100);
        ...
        public void ShowHObj()
        {
            ...
            if (mObj.hObjectSel_Info.hObjSelected != null)
            {
                HOperatorSet.DispObj(mObj.hObjectSel_Info.hObjSelected, mWindowH.HWindowHalconID);
                mWindowH.DispObj(mObj.hObjectSel_Info.hObjSelected);
            }
        }
        ...
        public override void FormToObj()
        {
            ...
            mObj.inputHobjName = ldHObj.TextInfo.Trim();

            string[] str = tbxIdx.Text.Trim().Split(',');
            for (int i = 0; i < str.Length; i++)
            {
                num.Append(str[i].ToInt());
            }
            mObj.num = num;
        ...
        public override void ObjToForm()
        {
            ...
            ldHObj.TextInfo = mObj.inputHobjName;

            for (int i = 0; i < mObj.num.Length; i++)
            {
                if (i == 0)
                {
                    str = mObj.num[i].D.ToString();
                }
                else
                {
                    str = str + "," + mObj.num[i].D.ToString();
                }
            }
            tbxIdx.Text = str;
        ...
        private void ldData_LinkData(object sender, EventArgs e)
        {
            ...
            switch (myLinkData.Name)
            {
                ...
                case "ldHObj":
                    ...
                    {
                        string[] m_IntStr = LikeDataForm.m_OutLinkData.Split('|');
                        ldHObj.TextInfo = m_IntStr[0] + ":" + m_IntStr[1];
                    }
                    break;
            }
        }

...
namespace Plugin.Break
{
    [Category("逻辑工具")]
    [DisplayName("退出循环")]
    [Serializable]
    public class BreakObj : ObjBase
    {
        #region 字段、属性
        ...
        public override bool RunObj()
        {
            try
            {
                ...
                SetVarList(ref mSloVar, new DataCell(ModInfo.Name, ModInfo.Encode, DataType.单量, DataMode.布尔型, ConstVar.If, ModInfo.Plugin, "0", 1, data, DataAtrr.全局变量));
        ...
        public void GetData()
        {
            if (dataMed.Contains(":"))
            {
                data = bool.Parse((string)Var.GetLinkValue(mSysVar, mSloVar, dataMed));
            }
            else
            {
        ...
        {
            new BreakForm((BreakObj)baseMod).ShowDialog();
        }

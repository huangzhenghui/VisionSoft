...
namespace Plugin.ReduceImage
{
    ...
    [DisplayName("掩膜抠图")]
    [Serializable]
    public class ReduceImageObj : ObjBase
    {
        #region 字段、属性
        ...
        /// 信息输出
        /// </summary>
        public ReduceImage_Info reduceImage_Info = new ReduceImage_Info();
        ...
        public override bool RunObj()
        {
            try
            {
                ...
                //更新数据
                reduceImage_Info.imageObj = RImage.ToRImage(new HImage(img));
               
                //主界面窗体显示
                WindowShowROI();

                //保存对象
                SetVarList(ref mSloVar, new DataCell(ModInfo.Name, ModInfo.Encode, DataType.单量, DataMode.掩膜抠图, ConstVar.ReduceImage, ModInfo.Plugin, "0", 1, reduceImage_Info, DataAtrr.全局变量));
        ...
        {
            new ReduceImageForm((ReduceImageObj)baseMod).ShowDialog();
        }
        ...
        {
            if (reduceImage_Info.imageObj != null && isShowReg)
            {
                reduceImage_Info.imageObj.Screen = screenIndex;
            }
        }
        ...
        {
            reduceImage_Info.imageObj = new RImage();
        }

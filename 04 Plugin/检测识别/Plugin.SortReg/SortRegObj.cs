...
namespace Plugin.SortReg
{
    ...
    [DisplayName("区域排序")]
    [Serializable]
    public class SortRegObj : ObjBase
    {
        #region 字段、属性
        ...
        /// 信息输出
        /// </summary>
        public SortReg_Info sortReg_Info = new SortReg_Info();
        ...
        /// 排序方式
        /// </summary>
        public string sortMode = "character";

        /// <summary>
        /// 增减顺序
        /// </summary>
        public string order = "true";

        /// <summary>
        /// 行列顺序
        /// </summary>
        public string rowOrCol = "row";
        ...
        public override bool RunObj()
        {
            try
            {
                ...
                //执行算法
                HOperatorSet.SortRegion(inputReg, out outReg, sortMode, order, rowOrCol);

                //输出数据更新
                sortReg_Info.resultReg = outReg;

                //主界面窗体显示
                WindowShowHObj();

                //保存对象
                SetVarList(ref mSloVar, new DataCell(ModInfo.Name, ModInfo.Encode, DataType.单量, DataMode.区域排序, ConstVar.SortReg, ModInfo.Plugin, "0", 1, sortReg_Info, DataAtrr.全局变量));
        ...
        {
            new SortRegForm((SortRegObj)baseMod).ShowDialog();
        }
        ...
        public void WindowShowHObj()
        {
            if (mRImage != null)
            {
                if (isShowReg)
                {
                    HRoi roiResult = new HRoi(ModInfo.Encode, ModInfo.Name, ModInfo.Remarks, HRoiType.检测结果, resultColor, new HObject(sortReg_Info.resultReg));

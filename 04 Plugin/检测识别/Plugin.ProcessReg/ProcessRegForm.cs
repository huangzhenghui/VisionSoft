...
{
    public partial class ProcessRegForm : FormBase
    {
        #region 字段、属性
        ...
        public ProcessRegForm(ProcessRegObj Obj) : base(Obj)
        {
            InitializeComponent();
        ...
        public void ShowHObj()
        {
            ...
            if (mObj.processReg_Info.regionObj != null)
            {
                HOperatorSet.DispObj(mObj.processReg_Info.regionObj, mWindowH.HWindowHalconID);
                mWindowH.DispObj(mObj.processReg_Info.regionObj);
            }
        }
        ...
        public override void FormToObj()
        {
            ...
            mObj.elementName = ldElement.TextInfo.Trim();
            mObj.processMode = cbxMode.Text;
            mObj.shape = cbxShape.Text;
            mObj.radius = tbxRadius.TextInfo.Trim().ToDouble();
            mObj.width = tbxWidth.TextInfo.Trim().ToDouble();
            mObj.height = tbxHeight.TextInfo.Trim().ToDouble();
        ...
        public override void ObjToForm()
        {
            ...
            cbxMode.Text = mObj.processMode;
            cbxShape.Text = mObj.shape;
            tbxWidth.TextInfo = mObj.width.ToString();
            tbxHeight.TextInfo = mObj.height.ToString();
            tbxRadius.TextInfo = mObj.radius.ToString();
            ldElement.TextInfo = mObj.elementName;
        ...
        {
            if (cbxShape.SelectedText == "矩形1")
            {
                pnlCirCleParams.Visible = false;
                pnlRect1Params.Visible = true;
                pnlCustomParams.Visible = false;
            }
            else if(cbxShape.SelectedText == "自定义")
            {
                pnlCirCleParams.Visible = false;
                pnlRect1Params.Visible = false;
                pnlCustomParams.Visible = true;
            }
            else
            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ConnectRegForm_Load(object sender, EventArgs e)
        {
            mWindowH.Draw = (ShowDraw)Enum.Parse(typeof(ShowDraw), cbxDraw.Text);

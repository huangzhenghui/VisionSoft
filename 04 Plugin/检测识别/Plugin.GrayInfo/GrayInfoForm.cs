...
{
    public partial class GrayInfoForm : FormBase
    {
        #region 字段、属性
        ...
        public GrayInfoForm(GrayInfoObj Obj) : base(Obj)
        {
            InitializeComponent();
        ...
        public void ShowHObj()
        {
            ...
            for (int i = 0; i < mObj.grayInfo_Info.mean.Length; i++)
            {
                if (i == 0)
                {
                    str1 = "灰度均值" + ":" + mObj.grayInfo_Info.mean[i].D.ToString("F4");
                    str2 = "灰度方差" + ":" + mObj.grayInfo_Info.deviation[i].D.ToString("F4");
                }
                else
                {
                    str1 = str1 + "," + mObj.grayInfo_Info.mean[i].D.ToString("F4");
                    str2 = str2 + "," + mObj.grayInfo_Info.deviation[i].D.ToString("F4");
                }
            }

            str = str1 + "\n" + str2;
            ShowTool.SetFont(mWindowH.hControl.HalconWindow, mObj.fontSize, mObj.fontType, "false", "false");
            ShowTool.SetMsg(mWindowH.hControl.HalconWindow, str, "window", 10, 10, mObj.resultColor, "false");
        }

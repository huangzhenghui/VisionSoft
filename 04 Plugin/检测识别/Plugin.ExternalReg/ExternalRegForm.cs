...
{
    public partial class ExternalRegForm : FormBase
    {
        #region 字段、属性
        ...
        public ExternalRegForm(ExternalRegObj Obj) : base(Obj)
        {
            InitializeComponent();
        ...
        public void ShowHObj()
        {
            ...
            HOperatorSet.ConcatObj(mObj.externalReg_Info.regionObj, mObj.inputReg, out objAssem);

            HOperatorSet.SetDraw(mWindowH.HWindowHalconID, mObj.showDraw);
            if (objAssem != null)
            {
                HOperatorSet.DispObj(objAssem, mWindowH.HWindowHalconID);
                mWindowH.DispObj(objAssem, mObj.showColor);
            }
        }

...
namespace Plugin.ConnectReg
{
    [Category("检测识别")]
    [DisplayName("区域连通")]
    [Serializable]
    public class ConnectRegObj : ObjBase
    {
        #region 字段、属性
        ...
        /// 阈值分割类
        /// </summary>
        public ConnectReg_Info connectReg_Info = new ConnectReg_Info();
        ...
        public override bool RunObj()
        {
            try
            {
                ...
                //执行算法
                HOperatorSet.Connection(inputReg, out connectReg_Info.regionObj);
                HOperatorSet.CountObj(connectReg_Info.regionObj, out regionNum);
                connectReg_Info.regNum = regionNum;

                //主界面窗体显示
                WindowShowHObj();

                //保存对象
                SetVarList(ref mSloVar, new DataCell(ModInfo.Name, ModInfo.Encode, DataType.单量, DataMode.区域连通, ConstVar.ConnectReg, ModInfo.Plugin, "0", 1, connectReg_Info, DataAtrr.全局变量));
        ...
        {
            HOperatorSet.GenEmptyObj(out connectReg_Info.regionObj);
        }
        ...
        {
            new ConnectRegForm((ConnectRegObj)baseMod).ShowDialog();
        }
        ...
        public void WindowShowHObj()
        {
            if (isShowHObj)
            {
                HRoi resultRegion = new HRoi(ModInfo.Encode, ModInfo.Name, ModInfo.Remarks, HRoiType.检测结果, showColor, connectReg_Info.regionObj);

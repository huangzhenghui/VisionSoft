...
{
    public partial class FrmSerialPortDebug : Form
    {
        #region 字段、属性
        ...
        public List<ECom> serialList;

        #region 初始化

        public FrmSerialPortDebug()
        {
            InitializeComponent();
        ...
        private void cliAreaBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                case "发送":
                    ECom.mSerialPort.SendData(tbxSend.Text);
                    break;
                case "清空":
                    tbxReceive.Text = "";
                    tbxSend.Text = "";
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FrmSerialPortDebug_Shown(object sender, EventArgs e)
        {
            thReceive = new Thread(new ThreadStart(() =>
            {
                while (true)
                {
                    try
                    {
                        ECom.mSerialPort.ReceiveData();
                        string msg = ECom.mSerialPort.readData;
                        if (msg != "")
                        {
                            Invoke(new Action(() =>
                            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FrmSerialPortDebug_FormClosed(object sender, FormClosedEventArgs e)
        {
            lock (ECom.mSerialPort.lockObj)
            {

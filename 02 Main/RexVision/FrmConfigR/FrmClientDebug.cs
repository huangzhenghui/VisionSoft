...
{
    public partial class FrmClientDebug : Form
    {
        #region 字段、属性
        ...
        private void cliAreaBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                case "发送":
                    ECom.mClient.SendData(tbxSend.Text);
                    break;
                case "清空":
                    tbxReceive.Text = "";
                    tbxSend.Text = "";
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FrmClientDebug_Shown(object sender, EventArgs e)
        {
            thReceive = new Thread(new ThreadStart(() =>
            {
                while (true)
                {
                    try
                    {
                        ECom.mClient.ReceiveData();
                        string msg = ECom.mClient.readData;
                        if (msg != "")
                        {
                            Invoke(new Action(() =>
                            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FrmClientDebug_FormClosed(object sender, FormClosedEventArgs e)
        {
            thReceive.Abort();
        }

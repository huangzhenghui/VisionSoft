...
namespace VisionCore
{
    ...
    public partial class ModTree : UserControl
    {
        ...
        public int ItemHeight { get; } = 0x20;
        /// <summary>
        /// 获取或设置每个子树节点级别的缩进距离。
        /// </summary>
        public int Indent
        ...
        public void Init(Dictionary<string, PluginsInfo> pluginsInfoDic)
        {
            ...
            _treeView.Nodes.Add("图像处理", "图像处理");//需要添加key Add(string key, string text);
            _treeView.Nodes.Add("检测识别", "检测识别");
            _treeView.Nodes.Add("几何测量", "几何测量");
            _treeView.Nodes.Add("几何构建", "几何构建");
            _treeView.Nodes.Add("定位工具", "定位工具");
            _treeView.Nodes.Add("读码工具", "读码工具");
            _treeView.Nodes.Add("变量工具", "变量工具");
            _treeView.Nodes.Add("数据工具", "数据工具");
            _treeView.Nodes.Add("逻辑工具", "逻辑工具");
            _treeView.Nodes.Add("通讯工具", "通讯工具");
            _treeView.Nodes.Add("流程工具", "流程工具");
            _treeView.Nodes.Add("脚本工具", "脚本工具");
            _treeView.Nodes.Add("文件工具", "文件工具");
        ...
        {
            int x = ((nest * this.ItemHeight) + (this.ItemHeight / 2)) - 8;
            int num2 = (x + (this.ItemHeight / 2)) + 8;
            int num3 = (num2 + this.ItemHeight)+3;
            int num4 = num2 + this.ItemHeight - 4;
            ...
            if (this.iconDefult != null)
            {
                ...
                if (image != null)
                {
                    g.DrawIcon(image, new Rectangle(num2, top+2, this.ItemHeight-8, this.ItemHeight-8));
                }
            }
            //画树结构文字 如果选中就添加蓝色底色
            if (node.Equals(this._treeView.SelectedNode))
            {
                Font = new Font("隶书", 12, FontStyle.Regular);
                g.FillRectangle(new SolidBrush(Color.Lavender), new Rectangle(num3-5, top + 1, base.Width, this.ItemHeight - 2));
                g.DrawString(" " + node.Text, Font, new SolidBrush(Color.Navy), (float)num4-8, (float)(num6 - (height / 2)));
            }
            else
            {
                g.DrawString(" " + node.Text, Font, new SolidBrush(Color.Navy), (float)num4-8, (float)(num6 - (height / 2)));
        ...
        private void ModTree_Load(object sender, EventArgs e)
        {
